...
class ColorsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create color" do
    assert_difference('Color.count') do
      post colors_url, params: { color: { cat_id: @color.cat_id, img_src: @color.img_src, name: @color.name } }, as: :json
    end
  ...
  test "should update color" do
    patch color_url(@color), params: { color: { cat_id: @color.cat_id, img_src: @color.img_src, name: @color.name } }, as: :json
    assert_response 200
  end
